...
class DeepQNetwork(object):
    def __init__(self, lr, n_actions, name, fc1_dims=1024,
                 input_dims=(80,60,4), chkpt_dir='tmp/dqn'):
        self.lr = lr
    ...
    def build_network(self):
        with tf.compat.v1.variable_scope(self.name):
            ...
            conv1 = tf.compat.v1.layers.conv2d(inputs=self.input, filters=32,
                                     kernel_size=(8,8), strides=4, name='conv1',
                     kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2))
            conv1_activated = tf.nn.relu(conv1)


            conv2 = tf.compat.v1.layers.conv2d(inputs=conv1_activated, filters=64,
                                     kernel_size=(4,4), strides=2, name='conv2',
                      kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2))
            conv2_activated = tf.nn.relu(conv2)


            conv3 = tf.compat.v1.layers.conv2d(inputs=conv2_activated, filters=128,
            ...
            self.loss = tf.compat.v1.reduce_mean(tf.square(self.Q_values - self.q_target))
...
class Agent(object):
    def __init__(self, alpha, gamma, mem_size, n_actions, epsilon, batch_size,
                 replace_target=10000, input_dims=(80,60,4),
